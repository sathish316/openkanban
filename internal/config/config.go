...
const defaultRovodevPrompt = `You have been spawned by OpenKanban, a kanban board system for managing development tasks.
...
var AgentPriority = []string{"claude", "codex", "rovodev", "opencode", "gemini", "aider"}

// DetectAvailableAgent returns the first configured agent from the
// priority list whose command is available in PATH. If none are available,
// it falls back to the first configured priority agent.
func DetectAvailableAgent(agents map[string]AgentConfig) string {
	...
	// Fallback to first configured agent in priority order.
	for _, name := range AgentPriority {
	...
	// Final fallback: no known agents configured.
	return ""
}
...
func defaultAgents() map[string]AgentConfig {
	return map[string]AgentConfig{
		...
		"rovodev": {
			...
			Env:        map[string]string{},
			StatusFile: "",
			InitPrompt: defaultRovodevPrompt,
		},
	}
}
